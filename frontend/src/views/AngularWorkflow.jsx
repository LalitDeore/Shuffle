...
export function SetJsonDotnotation(jsonInput, inputKey) {
  ...
  if (Array.isArray(jsonInput)) {
    ...
    for (var key in jsonInput) {
      ...
      if (Array.isArray(value)) {
        for (var i = 0; i < value.length; i++) {
          jsonInput[key][i] = SetJsonDotnotation(
            jsonInput[key][i],
            inputKey + "." + key + ".#"
          );
        }
      } else if (typeof value === "object") {
        jsonInput[key] = SetJsonDotnotation(
          jsonInput[key],
          inputKey + "." + key
        );
      } else {
    ...
    console.log(
      "SetJsonDotnotation: jsonInput is not an object or array, but key ",
      jsonInput,
      typeof jsonInput
    );
  }
...
const AngularWorkflow = (defaultprops) => {
  ...
  const [leftViewOpen, setLeftViewOpen] = React.useState(
    isMobile ? false : true
  );
  ...
  const [showWorkflowRevisions, setShowWorkflowRevisions] =
    React.useState(false);
  ...
  const [editWorkflowModalOpen, setEditWorkflowModalOpen] =
    React.useState(false);
  ...
  const [defaultEnvironmentIndex, setDefaultEnvironmentIndex] =
    React.useState(0);
  const [workflowRecommendations, setWorkflowRecommendations] =
  ...
  const [tenzirConfigModalOpen, setTenzirConfigModalOpen] =
    React.useState(false);
          ...
          parameters: [
            {
              name: "action",
              value: "list_tickets",
              options: ["list_tickets", "get_ticket", "create_ticket"],
              required: true,
            },
            {
              name: "fields",
          ...
          parameters: [
            {
              name: "action",
              value: "list_messages",
              options: ["list_messages", "send_message"],
              required: true,
            },
            {
              name: "fields",
    ...
    fetch(appUrl, {
      ...
      .then((responseJson) => {
        ...
        if (
          ...
          responseJson.app !== undefined &&
          responseJson.app !== null &&
          responseJson.app.length > 0
        ) {
          // Base64 decode into json
          const foundapp = JSON.parse(atob(responseJson.app));
          const selectedAppActions =
            selectedApp.actions === undefined || selectedApp.actions === null
              ? []
              : selectedApp.actions;
          if (
            foundapp.actions !== undefined &&
            foundapp.actions !== null &&
            foundapp.actions.length > selectedAppActions.length
          ) {
            if (select) {
  ...
  useEffect(() => {
    if (
      ...
      (selectedApp === null && selectedApp.app_name === undefined)
    ) {
      return;
    }
    ...
    if (
      selectedApp.actions === undefined ||
      selectedApp.actions === null ||
      selectedApp.actions.length > 1
    ) {
      return;
    } else {
      if (
        selectedApp.id !== undefined &&
        selectedApp.id !== null &&
    ...
    for (let appkey in apps) {
      ...
      if (
        curapp.actions !== undefined &&
        curapp.actions !== null &&
        curapp.actions.length > selectedApp.actions.length
      ) {
        ...
        for (let actionkey in curapp.actions) {
          ...
          if (
            curaction.category_label !== undefined &&
            curaction.category_label !== null &&
            curaction.category_label.length > 0
          ) {
  ...
  const [executionArgumentModalOpen, setExecutionArgumentModalOpen] =
    React.useState(false);
  ...
  const cytoscapeWidth = isMobile
    ? bodyWidth - leftBarSize
    : bodyWidth - leftBarSize - 25;
  ...
  */
  ...
  useEffect(() => {
    if (
      ...
      workflow.id === originalWorkflow.id
    ) {
    ...
    fetch(
      ...
      .then((responseJson) => {
        if (responseJson.success === true) {
          if (
            responseJson.meta !== undefined &&
            responseJson.meta !== null &&
            Object.getOwnPropertyNames(responseJson.meta).length > 0
          ) {
          ...
          if (
            ...
            if (
              !responseJson.reason.includes("404: Not Found") &&
              responseJson.reason.length > 25
            ) {
  ...
  useEffect(() => {
    if (
      ...
      selectedAction.app_name !== undefined
    ) {
      ...
      if (
        selectedAction.documentation === undefined ||
        selectedAction.documentation === null ||
        selectedAction.documentation.length === 0
      ) {
        ...
        if (responseJson !== undefined) {
            ...
            if (trigger.parameters.length >= 3) {
              for (let paramkey in trigger.parameters) {
                ...
                if (param.name === "workflow" || param.name === "subflow") {
                  ...
                  if (
                    workflow.triggers[trigger_index].parameters[paramIndex]
                ...
                if (
                  ...
                  if (Object.getOwnPropertyNames(baseSubflow).length > 0) {
                    const foundAction = baseSubflow.actions.find(
                      (action) => action.id === param.value
                    );
          ...
          if (workflows.length === 0) {
              ...
              for (let triggerkey in innerworkflow.triggers) {
                ...
                if (
                  ...
                  for (let paramkey in trigger.parameters) {
                    const param = trigger.parameters[paramkey];
                    if (
                      (param.name === "workflow" || param.name === "subflow") &&
                      param.value === props.match.params.key &&
                      !parent_ids.includes(innerworkflow.id)
                    ) {
            ...
            if (parentworkflows.length > 0) {
              setParentWorkflows(
                parentworkflows.filter((wf) => wf.id !== props.match.params.key)
              );
            }
          }
    ...
    fetch(`${globalUrl}/api/v2/workflows/${id}/executions`, {
      ...
      .then((responseJson) => {
        if (
          ...
          responseJson !== null &&
          responseJson.executions !== undefined &&
          responseJson.executions !== null
        ) {
          // - means it's opposite
          ...
          if (tmpView !== undefined && tmpView !== null && tmpView.length > 0) {
            ...
            const execution = responseJson.executions.find(
            ...
            if (execution !== null && execution !== undefined) {
              ...
              console.log(
                "Couldn't find execution for execution ID. Retrying as user to get ",
                tmpView
              );
  ...
  const handleSubmit = (trigger) => {
    ...
    if (selectedOption === "Kafka Queue") {
      ...
      const bootstrapServers =
        document.getElementById("bootstrap_servers")?.value;
      const groupId = document.getElementById("group_id")?.value;
      const autoOffsetReset =
        document.getElementById("auto_offset_reset")?.value;
  ...
  const handleUpdateResults = (responseJson, executionRequest) => {
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      ...
      if (responseJson.results !== null && responseJson.results.length > 0) {
        ...
        if (
          responseJson.workflow.actions !== undefined &&
          responseJson.workflow.actions !== null
        ) {
  ...
  const saveWorkflow = (
        ...
        if (type === "ACTION") {
          ...
          if (
            ...
            if (cyelements[cyelementsKey].data().example !== undefined) {
              curworkflowAction.example =
                cyelements[cyelementsKey].data().example;
            }
          }
          ...
          for (let parametersKey in curworkflowAction.parameters) {
            ...
            if (
              thisitem.value !== undefined &&
              thisitem.value !== null &&
              Array.isArray(thisitem.value)
            ) {
          ...
          if (
            useworkflow.triggers === undefined ||
            useworkflow.triggers === null
          ) {
          ...
          if (
            useworkflow.comments === undefined ||
            useworkflow.comments === null
          ) {
          ...
          if (curworkflowComment === undefined) {
            ...
            try {
              curworkflowComment.position.x = parseInt(
                curworkflowComment.position.x
              );
            } catch (e) {
              console.log(
                "Failed to parse position Y of comment: ",
                curworkflowComment.position.x
              );
            }

            try {
              curworkflowComment.position.y = parseInt(
                curworkflowComment.position.y
              );
            } catch (e) {
              console.log(
                "Failed to parse position Y of comment: ",
	...
	*/
    ...
    if (cy !== undefined) {
      ...
      if (
        cyImageData !== undefined &&
        cyImageData !== null &&
        cyImageData.length > 0
      ) {
    ...
    if (
      useworkflow.id === undefined ||
      useworkflow.id === null ||
      useworkflow.id.length === 0
    ) {
    ...
    if (
      useworkflow.org_id !== undefined &&
      useworkflow.org_id !== null &&
      useworkflow.org_id.length > 0
    ) {
	...
	*/
    ...
    fetch(`${globalUrl}/api/v1/workflows/${useworkflow.id}`, {
      ...
      .then((responseJson) => {
        if (
          useworkflow.id === originalWorkflow.id &&
          duplicationOrg !== undefined &&
          duplicationOrg !== null &&
          duplicationOrg.length > 0
        ) {
        ...
        if (!responseJson.success) {
          ...
          if (
            ...
            toast(
              "Failed to save. Please contact your support@shuffler.io or your local admin if this is unexpected."
            );
          }
        } else {
          sendStreamRequest({
    ...
    if (
      originalWorkflow.id === undefined ||
      originalWorkflow.id === null ||
      originalWorkflow.id.length === 0 ||
      useworkflow.id === originalWorkflow.id
    ) {
  ...
  const executeWorkflow = (executionArgument, startNode, hasSaved) => {
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      ...
      if (
        executionArgument === undefined ||
        executionArgument === null ||
        executionArgument.length === 0
      )
        if (
          ...
          for (let actionkey in workflow.actions) {
            if (
              workflow.actions[actionkey].parameters === undefined ||
              workflow.actions[actionkey].parameters === null ||
              workflow.actions[actionkey].parameters.length === 0
            ) {
              continue;
            }
          ...
          if (foundmissing) {
            ...
            if (
              workflow.input_questions !== undefined &&
              workflow.input_questions !== null &&
              workflow.input_questions.length > 0
            ) {
              setExecutionRequestStarted(false);
      ...
      if (
        workflow.org_id !== undefined &&
        workflow.org_id !== null &&
        workflow.org_id.length > 0
      ) {
        ...
        .then((responseJson) => {
          if (!responseJson.success) {
            ...
            if (
              ...
              responseJson.execution_id !== null &&
              responseJson.execution_id.length > 0
            ) {
          ...
          if (updateAction === true) {
            if (selectedApp.authentication.required) {
              ...
              for (let authkey in tmpAuth) {
                ...
                if (itemname === appname) {
                  ...
                  if (item.edited > latest) {
                    ...
                    for (let actionkey in workflow.actions) {
                      const actionAppname = workflow.actions[actionkey].app_name
                        .toLowerCase()
                        .replaceAll(" ", "_", -1);
                      if (actionAppname === appname) {
                        workflow.actions[actionkey].selectedAuthentication =
                          item;
              ...
              if (appUpdates === true) {
                ...
                toast(
                  "Failed to find new authentication. See details in Oauth2 popup window where auth was attempted."
                );
                shouldClose = false;
              }
            } else {
    ...
    fetch(globalUrl + "/api/v1/apps", {
      ...
      .then((response) => {
        if (response.status !== 200) {
          ...
          if (isLoggedIn) {
            toast(
              "Something went wrong while loading apps. Please refresh the window to try again."
            );
          }
      ...
      .then((responseJson) => {
        ...
        const foundTools = responseJson.find(
          (app) => app.name === "Shuffle Tools"
        );
        ...
        var handledPrioritizedApps = responseJson.filter((app) =>
          internalIds.includes(app.name.toLowerCase())
        );
        handledPrioritizedApps = [].concat(
          integrationApps,
          handledPrioritizedApps
        );

        if (isCloud) {
          setFilteredApps(
            responseJson.filter(
              (app) => !internalIds.includes(app.name.toLowerCase())
            )
          );
          setPrioritizedApps(handledPrioritizedApps);
        } else {
          const tmpFiltered = responseJson.filter(
            (app) => !internalIds.includes(app.name.toLowerCase())
          );
    ...
    fetch(`${globalUrl}/api/v1/users/creators/${username}`, {
      ...
      .then((responseJson) => {
        if (responseJson.success !== false) {
          ...
          console.log(
            "Couldn't find the creator profile (rerun?): ",
            responseJson,
            rerun
          );
  ...
  const moveNode = (chunkJson) => {
    ...
    const connectedNodes = cy.filter(
      'node[attachedTo = "' + chunkJson.id + '"]'
    );
  ...
  const addNode = (chunkJson) => {
    ...
    setTimeout(() => {
      ...
      if (node === undefined || node === null) {
        console.log(
          "Node is undefined or null during auto add from other user"
        );
        return;
      }
  ...
  const getChildWorkflows = (parentWorkflowId) => {
    if (
      originalWorkflow.suborg_distribution === undefined ||
      originalWorkflow.suborg_distribution === null ||
      originalWorkflow.suborg_distribution.length === 0
    ) {
      return;
    }

    const orgId =
      originalWorkflow.org_id === undefined ||
      originalWorkflow.org_id === null ||
      originalWorkflow.org_id === ""
        ? ""
        : originalWorkflow.org_id;
  ...
  const getWorkflow = (workflow_id, sourcenode) => {
    fetch(`${globalUrl}/api/v1/workflows/${workflow_id}`, {
      ...
      .then((response) => {
        if (response.status !== 200) {
          ...
          if (response.status >= 500) {
            toast(
              "Something went wrong while loading the workflow. Please reload."
            );
          } else {
            ...
            var sessionToken = new URLSearchParams(cursearch).get(
              "session_token"
            );
            if (execFound === null && sessionToken === null) {
              toast(
                `You don't have access to this workflow or loading failed. Redirecting to workflows in a few seconds..`
              );
              setTimeout(() => {
              ...
              sessionToken !== null &&
              workflow_id === "3abdfb21-b40f-4e50-b855-ac0d62f83cbe"
            ) {
              toast(`Injecting session token and reloading workflow..`);
              setTimeout(() => {
                setCookie("session_token", sessionToken, { path: "/" });
                window.location.href =
                  "https://shuffler.io/workflows/3abdfb21-b40f-4e50-b855-ac0d62f83cbe";
              }, 2000);
            }
          }
        }
      ...
      .then((responseJson) => {
        ...
        if (
          responseJson.id !== undefined &&
          responseJson.id !== null &&
          responseJson.id.length > 0 &&
          responseJson.id !== workflow_id
        ) {
        ...
        if (
          responseJson.parentorg_workflow !== undefined &&
          responseJson.parentorg_workflow !== null &&
          responseJson.parentorg_workflow !== ""
        ) {
          setDistributedFromParent(responseJson.parentorg_workflow);
        }

        if (
          responseJson.childorg_workflow_ids !== undefined &&
          responseJson.childorg_workflow_ids !== null &&
          responseJson.childorg_workflow_ids.length > 0
        ) {
        ...
        if (
          responseJson.parentorg_workflow !== undefined &&
          responseJson.parentorg_workflow !== null &&
          responseJson.parentorg_workflow.length > 0
        ) {
        ...
        if (
          responseJson.triggers === undefined ||
          responseJson.triggers === null
        ) {
        ...
        if (responseJson.public) {
          ...
          for (let actionkey in responseJson.actions) {
            ...
            if (
              parsedAction.large_image === undefined ||
              parsedAction.large_image === null ||
              parsedAction.large_image === ""
            ) {
              continue;
            }
            if (
              appsFound.findIndex(
                (data) => data.app_name === parsedAction.app_name
              ) < 0
            ) {
        ...
        if (sourcenode.id !== undefined) {
          var nodefound = false;
          var target = sourcenode.parameters.find(
  ...
  const onUnselect = (event) => {
    ...
    for (var idkey in typeIds) {
      ...
      if (item.data.isButton === true) {
        ...
        if (
          item.data.buttonType === "delete" &&
          item.data.attachedTo === nodedata.id
        ) {
  ...
  const onCtxTap = (event) => {
    ...
    if (
      ...
      const workflow_id = nodedata.parameters.find(
        (param) => param.name === "workflow" || param.name === "subflow"
      );
  ...
  const onNodeDragStop = (event, selectedAction) => {
    ...
    if (connected.length > 0 && connected !== undefined) {
      for (let connectkey in connected) {
        ...
        const edgeCurve = calculateEdgeCurve(
          sourcenode.position(),
    ...
    if (
      ...
      if (
        ...
        nodedata.app_name === "Gmail" ||
        nodedata.app_name === "Office365"
      ) {
        if (!found) {
          console.log(
            "Find amount of executions for the specific nodetype: ",
            nodedata.app_name,
            "Executions: ",
            workflowExecutions
          );
          ...
          var executions = 0;
          const matchingExecutions = workflowExecutions.filter(
            (execution) =>
              execution.execution_source === nodedata.app_name.toLowerCase()
          );
  ...
  const onNodeDrag = (event, selectedAction) => {
    ...
    if (
      (nodedata.trigger_type === "SUBFLOW" ||
        nodedata.trigger_type === "USERINPUT" ||
      ...
      for (var branchkey in branches) {
        if (
          branches[branchkey].data.source === nodedata.id ||
          branches[branchkey].data.target === nodedata.id
        ) {
      ...
      if (!branchFound) {
        ...
        for (var nodekey in allnodes) {
          ...
          if (decoratorIds.includes(node.data.id)) {
            ...
            if (distance > 300) {
              // Remove the branch
              const edgeToRemove = cy.getElementById(
                branches[branchkey].data.id
              );
      ...
      if (elementMouseIsOver !== undefined && elementMouseIsOver !== null) {
        ...
        if (
          ...
          for (var styledElementsKey in styledElements) {
            const curElement = document.getElementById(
              styledElements[styledElementsKey]
            );
            if (curElement !== null && curElement !== undefined) {
              curElement.style.border = curElement.style.original_border;
            }
  ...
  const aiSubmit = (
    ...
    fetch(`${globalUrl}/api/v1/conversation`, {
      ...
      .then((responseJson) => {
        ...
        if (inputAction !== undefined) {
          console.log(
            "In input action! Should check params if they match, and add suggestions"
          );

          if (
            responseJson.parameters === undefined ||
            responseJson.parameters.length === 0
          ) {
            return;
          }
          ...
          for (let paramkey in inputAction.parameters) {
            ...
            if (
              actionParam.configuration === true &&
              actionParam.name !== "url"
            ) {
              continue;
            }
            ...
            if (
              actionParam.value !== "" &&
              actionParam.value !== actionParam.example
            ) {
        ...
        // Add action
        if (
          responseJson.app_name !== undefined &&
          responseJson.app_name !== null
        ) {
          // Always added to 0, 0
          ...
          var newitem = {
            ...
            position: {
              x:
                suggestionBox.node_position.x !== undefined
                  ? suggestionBox.node_position.x
                  : 0,
              y:
                suggestionBox.node_position.y !== undefined
                  ? suggestionBox.node_position.y + 100
                  : 0,
            },
            group: "nodes",
          };
  ...
  const onNodeSelect = (event, newAppAuth) => {
    ...
    if (data.buttonType == "ACTIONSUGGESTION") {
        ...
        for (let appkey in apps) {
          ...
          for (let actionkey in curapp.actions) {
            ...
            if (
              ...
              if (
                curaction.category_label[0].toLowerCase() ===
                findaction.toLowerCase()
              ) {
                ...
                if (foundindex >= 0) {
                  ...
                  workflow.actions[foundindex].description =
                    curaction.description;
                  workflow.actions[foundindex].parameters =
      ...
      for (var _key in allNodes) {
        ...
        if (
          ...
          currentNode.data.attachedTo !== data.id
        ) {
          cy.getElementById(currentNode.data.id).remove();
        }
      }
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      if (data.isButton) {
        if (data.buttonType === "suggestion") {
          ...
          if (
            ...
            if (
              ...
              const svg_pin = `<svg width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}" version="1.1" xmlns="http://www.w3.org/2000/svg"><path d="${iconInfo.icon}" fill="${iconInfo.iconColor}"></path></svg>`;
              const svgpin_Url = encodeURI(
                "data:image/svg+xml;utf-8," + svg_pin
              );
            ...
            for (var sourceBranchesKey in sourcebranches) {
              var newbranch = JSON.parse(
                JSON.stringify(sourcebranches[sourceBranchesKey])
              );
            ...
            for (var destinationBranchesKey in destinationbranches) {
              ...
              if (sourcenode !== null && sourcenode !== undefined) {
                ...
                if (
                  sourcedata.trigger_type !== "SUBFLOW" &&
                  sourcedata.trigger_type !== "USERINPUT"
                ) {
                  continue;
                }
              }
        ...
        if (data.isTrigger) {
          console.log(
            "But maybe we can select trigger descriptor? Maybe open execution tab?"
          );
          setExecutionModalOpen(true);
        }
      ...
      if (data.type === "ACTION") {
        ...
        var curapp = newapps.find((a) => a.id === curaction.app_id);

        if (curapp === undefined || curapp === null) {
          console.log(
            "Couldn't find app with that ID - checking with name & version"
          );
        ...
        if (curaction.template === true && curaction.name !== undefined) {
          ...
          console.log(
            "FIND AN ACTION AMONG THE APPS THAT MATCHES NAME: ",
            parsedname
          );
          ...
          for (var newAppskey in newapps) {
            for (let actionsSubkey in newapps[newAppskey].actions) {
              ...
              if (
                tmpaction.name.replaceAll(" ", "_").toLowerCase() === parsedname
              ) {
		...
		*/
        ...
        if (!curapp || curapp === undefined) {
          ...
          if (
            curapp.authentication === undefined ||
            curapp.authentication === null
          ) {
            ...
            setAuthenticationType(
              curapp.authentication.type === "oauth2-app" ||
            ...
            for (let tmpAuthKey in tmpAuth) {
              ...
              for (let fieldkey in item.fields) {
                if (item.fields[fieldkey] === undefined) {
                  console.log(
                    "Problem with filterkey in Node select",
                    fieldkey
                  );
                  continue;
                }
          ...
          if (
            ...
            for (var curActionParamKey in curaction.parameters) {
              if (
                ...
                curaction.parameters[curActionParamKey].value =
                  curaction.parameters[curActionParamKey].options[0];
              }
            }
          } else {
            ...
            for (let actionKey in curapp.actions) {
              ...
              if (tmpaction.name === curaction.name) {
                ...
                if (
                  tmpaction.parameters !== undefined &&
                  tmpaction.parameters !== null &&
                  tmpaction.parameters.length > 0
                ) {
                  curaction.parameters = JSON.parse(
                    JSON.stringify(tmpaction.parameters)
                  );
                }
                break;
              }
        ...
        if (
          ...
          environments !== null &&
          (typeof environments === "array" || typeof environments === "object")
        ) {
        ...
        if (
          data.app_name === "Shuffle Workflow" ||
          data.app_name === "User Input"
        ) {
          ...
          if (
            ...
            workflow.triggers[trigger_index].parameters[0] = {
              name: "url",
              value:
                referenceUrl + "webhook_" + workflow.triggers[trigger_index].id,
            };
          ...
          if (
            ...
            data.environment === "" ||
            data.environment.toLowerCase() === "cloud"
          ) {
            for (var envKey in environments) {
              ...
              workflow.triggers[trigger_index].environment =
            ...
            if (trigger !== undefined && trigger !== null) {
              ...
              if (trigger.trigger_type === "USERINPUT") {
                ...
                for (var paramkey in trigger.parameters) {
                  if (
                    relevantparams.includes(trigger.parameters[paramkey].name)
                  ) {
                    foundparams++;
                  }
                }
                  ...
                  trigger.parameters = [
                    {
                      name: "alertinfo",
                      value:
                        "Do you want to continue the workflow? Start parameters: $exec",
                    },
                    {
                      name: "options",
                      value: "boolean",
  ...
  const activateApp = (appid, refresh) => {
    fetch(`${globalUrl}/api/v1/apps/${appid}/activate`, {
      ...
      .then((responseJson) => {
        if (responseJson.success === false) {
          toast(
            "Failed to auto-activate the app. Go to /apps and activate it."
          );
        } else {
          if (refresh === true) {
    ...
    if (parents.length > 1) {
      for (let parentkey in parents) {
        ...
        if (dstdata.parameters !== undefined && dstdata.parameters !== null) {
          for (let [paramkey, paramkeyval] in Object.entries(
            dstdata.parameters
          )) {
  ...
  const onEdgeAdded = (event) => {
    ...
    if (
      ...
      console.log(
        "Source or destination node is undefined or null: ",
        sourcenode,
        destinationnode
      );
    } else {
      console.log(
        "Edge added: Is it a trigger? If so, check if it already has a branch and remove it: ",
        sourcenode.data()
      );
      if (sourcenode.data("type") === "TRIGGER") {
        if (
          sourcenode.data("app_name") !== "Shuffle Workflow" &&
          sourcenode.data("app_name") !== "User Input"
        ) {
          setTimeout(() => {
            const alledges = cy.edges().jsons();
            var targetedge = alledges.findIndex(
              (data) =>
                data.data.source === edge.source && data.data.id !== edge.id
            );
    ...
    if (targetnode !== -1) {
      if (
        ...
        workflow.triggers[targetnode].app_name === "Shuffle Subflow"
      ) {
    ...
    for (let branchkey in workflow.branches) {
      ...
      if (
        ...
        console.log(
          ...
          workflow.branches[branchkey]
        );
        const foundbranch = cy.getElementById(workflow.branches[branchkey].id);
        if (
          foundbranch !== undefined &&
          foundbranch !== null &&
          foundbranch.data() !== undefined &&
          foundbranch.data() !== null
        ) {
  ...
  const onNodeAdded = (event) => {
    ...
    if (nodedata.type === "ACTION") {
      ...
      */
      ...
      if (
        edgeToBeAdded.data.source !== edgeToBeAdded.data.target &&
        edgeToBeAdded.data.source !== undefined &&
        edgeToBeAdded.data.target !== undefined
      ) {
        if (
  ...
  const onEdgeRemoved = (event) => {
    ...
    for (let nodekey in allNodes) {
      ...
      if (curnode.data.id === edge.data("source")) {
        ...
        if (
          curnode.data.app_name !== "Shuffle Workflow" &&
          curnode.data.app_name !== "User Input"
        ) {
          ...
          if (curnode.data.status === "running") {
            ...
            try {
              ...
              console.log(
                "You must STOP the trigger before deleting its branches"
              );
            } catch (e) {
  ...
  const addActionSuggestions = (nodedata, event) => {
    ...
    for (var appkey in apps) {
      ...
      if (
        curapp.actions === undefined ||
        curapp.actions === null ||
        curapp.actions.length === 0
      ) {
        continue;
      }

      var startIndex = curapp.actions.findIndex(
        (action) =>
          action.category_label !== undefined &&
          action.category_label !== null &&
          action.category_label.length > 0
      );
      if (startIndex === -1) {
        startIndex = 0;
      }
    ...
    for (let appKey in apps) {
      ...
      for (let actionKey in curapp.actions) {
        ...
        if (
          ...
          if (
            curaction.category_label[0].replaceAll("_", " ").toLowerCase() ===
            "no label"
          ) {
            continue;
          }
  ...
  const addSuggestionButtons = (nodedata, event) => {
    ...
    if (
      workflowRecommendations === undefined ||
      workflowRecommendations === null ||
    ...
    for (let recKey in workflowRecommendations) {
      ...
      for (let recIndex in rec.recommendations) {
        ...
        const foundVersion =
          parsedRec.app_version !== undefined &&
          parsedRec.app_version !== null &&
          parsedRec.app_version !== ""
            ? parsedRec.app_version
            : "1.1.0";
        const foundApp = apps.find(
          (app) =>
            app.app_name === parsedRec.app_name &&
            app.app_version === foundVersion
        );
        // Find out if foundApp is shuffle tools, and if so, add the correct image based on name

        const largeImage =
          parsedRec.large_image !== undefined &&
          parsedRec.large_image !== null &&
          parsedRec.large_image !== ""
            ? parsedRec.large_image
            : foundApp === undefined || foundApp === null
            ? theme.palette.defaultImage
            : foundApp.large_image;
  ...
  const onNodeHover = (event) => {
    ...
    if (nodedata.app_name !== undefined && !workflow.public === true) {
      ...
      for (var _key in allNodes) {
        ...
        if (
          (currentNode.data.buttonType === "ACTIONSUGGESTION" ||
            currentNode.data.isButton ||
  ...
  const calculateEdgeCurve = (sourcenodePosition, destinationnodePosition) => {
    ...
    const alpha = 0.3;
    var controlPointDistance = [
      -alpha * yParsed * costheta,
      alpha * yParsed * costheta,
    ];
  ...
  const setupGraph = (inputworkflow) => {
    ...
    if (cy !== undefined && cy !== null) {
      if (
        inputworkflow.actions !== undefined &&
        inputworkflow.actions !== null &&
        inputworkflow.actions.length > 0
      ) {
        //cy.remove('*')
      }
    }
    ...
    if (
      inputworkflow.branches === undefined ||
      inputworkflow.branches === null
    ) {
      inputworkflow.branches = [];
    }

    if (
      inputworkflow.triggers === undefined ||
      inputworkflow.triggers === null
    ) {
    ...
    const foundtriggers = inputworkflow.triggers.map((trigger) => {
      ...
      if (
        trigger.large_image === undefined ||
        trigger.large_image === null ||
    ...
    var edges = inputworkflow.branches.map((branch, index) => {
	  ...
	  */
      ...
      if (
        branch.parent_controlled !== undefined &&
        branch.parent_controlled !== null &&
        branch.parent_controlled === true
      ) {
      ...
      var sourcenode = actions.find(
        (node) =>
          node.data._id === branch.source_id ||
          node.data.id === branch.source_id
      );
      var destinationnode = actions.find(
        (node) =>
          node.data._id === branch.destination_id ||
          node.data.id === branch.destination_id
      );

      if (sourcenode === undefined) {
        sourcenode = foundtriggers.find(
          (node) =>
            node.data._id === branch.source_id ||
            node.data.id === branch.source_id
        );
      ...
      if (destinationnode === undefined) {
        destinationnode = foundtriggers.find(
          (node) =>
            node.data._id === branch.destination_id ||
            node.data.id === branch.destination_id
        );
  ...
  const removeNode = (nodeId) => {
    ...
    if (
      selectedNode.data().decorator === true &&
  ...
  if (firstrequest) {
    ...
    if (tmpExec !== undefined && tmpExec !== null && tmpExec === "executions") {
      setExecutionModalOpen(true);
  ...
  if (
    ...
    !established &&
    cy !== undefined &&
    ((apps !== null && apps !== undefined && apps.length > 0) ||
      workflow.public === true) &&
    Object.getOwnPropertyNames(workflow).length > 0 &&
    appAuthentication !== undefined
  ) {
    ...
    cy.on("boxselect", "node", (e) => {
      if (
        e.target.data("isButton") ||
        e.target.data("isDescriptor") ||
        e.target.data("isSuggestion")
      ) {
        e.target.unselect();
      }
  ...
  const parsedHeight = isMobile
    ? bodyHeight - appBarSize * 4
    : bodyHeight - appBarSize - 50;
  const appViewStyle = {
    marginLeft: 5,
    marginRight: 5,
    display: "flex",
    ...
    const deleteVariable = (type, variableIndex) => {
      ...
      if (type === "normal") {
        if (
          ...
          workflow.workflow_variables.length > variableIndex
        ) {
        ...
        if (
          ...
          workflow.execution_variables.length > variableIndex
        ) {
          ...
          {triggers.map((trigger, index) => {
			...
			*/
            ...
            const imagesize = isMobile
              ? 40
              : trigger.large_image.includes("svg")
              ? 50
              : 50;
            var imageline =
              trigger.large_image.length === 0 ? (
  ...
  const handleDragStop = (e, app) => {
    ...
    if (
      parsedApp === undefined ||
      parsedApp === null ||
      parsedApp.data === undefined ||
      parsedApp.data === null
    ) {
      toast("Failed to add node. Please try again.");
      console.log(
        "Failed to add node. Please try again. Parsed app:",
        parsedApp
      );
      return;
    }
    ...
    if (
      ...
      if (newNodeId.length > 0) {
		...
		*/
        ...
        const startIndex = app.actions.findIndex(
          (action) =>
            action.category_label !== undefined &&
            action.category_label !== null &&
            action.category_label.length > 0
        ...
        if (
          app.actions[actionIndex] === undefined ||
          app.actions[actionIndex] === null
        ) {
        ...
        if (
          ...
          for (let paramkey in parameters) {
            ...
            if (
              parameters[paramkey].name === "headers" &&
              parameters[paramkey].value.includes("=undefined")
            ) {
              if (
                parameters[paramkey].example !== undefined &&
                parameters[paramkey].example !== null &&
                parameters[paramkey].example.length > 0
              ) {
        ...
        if (
          ...
          for (var key in appAuthentication) {
            ...
            const appauthname = authKey.app.name
              .toLowerCase()
              .replace(" ", "_");
        ...
        if (cy !== undefined && cy !== null) {
          ...
          if (
            ...
            for (let nodekey in foundnodes) {
              ...
              if (
                curnode.data.environment !== undefined &&
                curnode.data.environment !== null &&
                curnode.data.environment.length > 0
              ) {
  ...
  const AppView = (props) => {
    ...
    const ParsedAppPaper = (props) => {
      ...
      if (
        app.large_image === undefined ||
        app.large_image === null ||
        app.large_image === ""
      ) {
        ...
        >
          <Paper
            ...
            onMouseOver={(e) => {
              ...
              if (
                app.actions !== undefined &&
                (app.actions === null || app.actions.length === 1)
              ) {
    ...
    const runSearch = (value) => {
      ...
      if (value.length > 0) {
        // Dedup based on name
        const preppedApps = Array.prototype.concat
          .apply(allApps, triggers)
          .filter((app, index, self) => {
            return index === self.findIndex((t) => t.name === app.name);
          });
    ...
    const SearchBox = ({ currentRefinement, refine, isSearchStalled }) => {
      if (document !== undefined) {
        ...
        if (appsearchValue !== undefined && appsearchValue !== null) {
          if (
            appsearchValue.value !== undefined &&
            appsearchValue.value !== null &&
            appsearchValue.value.length > 0
          ) {
      ...
      return (
        <div
          style={{
            ...
            zIndex: 1001,
            backgroundColor: theme.palette.inputColor,
            minWidth: leftBarSize - 10,
            maxWidth: leftBarSize - 10,
            boxShadows: "none",
            overflowX: "hidden",
          }}
        >
            ...
            {hits.length === 0 ? (
              ...
              hits.map((hit, index) => {
                const innerlistitemStyle = {
                  ...
                  borderBottom: "1px solid rgba(255,255,255,0.4)",
                  backgroundColor:
                    mouseHoverIndex === index ? "#1f2023" : "inherit",
                  cursor: "pointer",
                  marginLeft: 0,
                  marginRight: 0,
                  maxHeight: 75,
                ...
                const name =
                  hit.name === undefined
                    ? hit.filename.charAt(0).toUpperCase() +
                      hit.filename.slice(1).replaceAll("_", " ") +
                      " - " +
                      hit.title
                    : (
                ...
                var secondaryText =
                  hit.data !== undefined ? hit.data.slice(0, 40) + "..." : "";
                const avatar =
                  hit.image_url === undefined ? (
                    baseImage
                  ) : (
                    <Avatar src={hit.image_url} variant="rounded" />
                  );

                //console.log(hit)
                if (
                  hit.categories !== undefined &&
                  hit.categories !== null &&
                  hit.categories.length > 0
                ) {
                  secondaryText = hit.categories
                    .slice(0, 3)
                    .map((data, index) => {
                      if (index === 0) {
                        return data;
                ...
                var parsedUrl = isCloud
                  ? `/apps/${hit.objectID}`
                  : `https://shuffler.io/apps/${hit.objectID}`;
      ...
      <div style={appViewStyle}>
        <div style={{ flex: "1" }}>
          ...
          {visibleApps.length > extraApps.length ? (
              ...
              {visibleApps.length <= 2 ? (
                ...
                >
                  <Typography variant="body1" color="textSecondary">
                    Click one of the relevant public apps below to Activate it
                ...
                <div
                  style={{ marginLeft: 10, marginTop: 10, marginBottom: 100 }}
                >
                  <Typography variant="body1" color="textSecondary">
                    Apps need to be activated before they can be used. Search
                    from our 2500+ apps to activate them for your organisation.
                  </Typography>
                </div>
              )}
            </div>
            ...
            >
              <Typography variant="body1" color="textSecondary">
                Couldn't find the apps you were looking for? Searching
                unactivated apps. Click one of these apps to Activate it for
                your organisation.
              </Typography>
              <InstantSearch
                searchClient={searchClient}
  ...
  const setNewSelectedAction = (e) => {
    ...
    if (selectedApp.actions.length === 1) {
      ...
      const newApp = apps.find(
        (app) =>
          (app.name === selectedApp.name &&
            app.app_version !== selectedApp.app_version) ||
    ...
    if (newaction === undefined || newaction === null) {
      toast(
        "Failed to find the action you selected. Please try again or contact support@shuffler.io if it persists."
      );
      return;
    }

    if (workflow.actions !== undefined && workflow.actions !== null) {
      const foundInfo = workflow.actions.find(
        (ac) => ac.id === selectedAction.id
      );
    }
    ...
    if (
      oldaction.parameters !== undefined &&
      oldaction.parameters !== null &&
      newSelectedAction.parameters !== undefined &&
      oldaction.id === newSelectedAction.id
    ) {
      ...
      for (let [paramkey, paramkeyval] in Object.entries(
        ...
        if (
          newSelectedAction.parameters === undefined ||
          newSelectedAction.parameters === null
        ) {
          continue;
        }

        // Not doing options fields
        const newParamIndex = newSelectedAction.parameters.findIndex(
          (paramdata) => paramdata.name === param.name
        );
        if (newParamIndex < 0) {
          continue;
        }
        ...
        if (newSelectedAction.parameters[newParamIndex].name === "headers") {
          if (
            param.value !== undefined &&
            param.value !== null &&
            param.value.includes("=undefined")
          ) {
            if (
              newSelectedAction.parameters[newParamIndex].example !==
                undefined &&
              newSelectedAction.parameters[newParamIndex].example !== null
            ) {
    ...
    if (
      ...
      const foundActionIndex = workflow.actions.findIndex(
        (actiondata) => actiondata.id === newSelectedAction.id
      );
      if (foundActionIndex >= 0) {
    ...
    if (
      newSelectedAction.parameters !== undefined &&
      newSelectedAction.parameters !== null &&
      newSelectedAction.parameters.length > 0
    ) {
  ...
  const setTriggerFolderWrapperMulti = (event) => {
    ...
    if (
      (selectedTrigger.name === "Office365") & (value !== undefined) &&
      value !== null &&
      value.length > 1
    ) {
  ...
  const setTriggerOptionsWrapper = (value) => {
    if (
      selectedTrigger.parameters === null ||
      selectedTrigger.parameters === undefined
    ) {
      selectedTrigger.parameters = [
        { name: "", value: "" },
        { name: "", value: "" },
  ...
  if (executionArgumentModalOpen && workflowExecutions.length > 0) {
    for (let executionKey in workflowExecutions) {
      ...
      if (
        execution.execution_argument === undefined ||
        execution.execution_argument === null ||
        execution.execution_argument.length < 2
      ) {
        continue;
      }
  ...
  const handleAppAuthGroupCheckbox = (data) => {
    ...
    const foundIndex = workflow.auth_groups.findIndex(
      (auth) => auth === data.id
    );
    if (foundIndex >= 0) {
  ...
  const authgroupModal = (
    <Dialog
      ...
      onClose={() => {}}
    >
      <Tooltip title="Close window" placement="top" style={{ zIndex: 10011 }}>
        <IconButton
      ...
      <DialogContent>
        <Typography variant="body1" color="textSecondary">
          Authgroups are a way to control how a workflow runs. If chosen, the
          workflow will use the groups on the nodes that have them selected. If
          three are chosen, the workflows runs three times. This is an
          experimental MSSP feature to handle multiple environments.
        </Typography>
        ...
        {authGroups.map((data, index) => {
          var checked = false;
          if (
            workflow.auth_groups !== undefined &&
            workflow.auth_groups !== null
          ) {
          ...
          if (
            data === undefined ||
            data === null ||
            data.app_auths === undefined ||
            data.app_auths === null ||
            data.app_auths.length === 0
          ) {
            return null;
          }
          ...
          return (
            ...
            >
              <Checkbox checked={checked} name={data.label} />

              <Typography
                variant="body1"
                color="textSecondary"
                style={{ minWidth: 200, maxWidth: 200, marginRight: 10 }}
              >
                {data.label}
              </Typography>
              ...
              {data.app_auths.map((appAuth, index) => {
                if (
                  appAuth.app.large_image === undefined ||
                  appAuth.app.large_image === null ||
                  appAuth.app.large_image === ""
                ) {
                  const foundImage = appAuthentication.find(
                    (auth) => auth.app.id === appAuth.app.id
                  );
                ...
                const tooltip = `${appAuth.app.name.replaceAll(
                  "_",
                  " "
                )} (authname: ${appAuth.label})`;
    ...
    >
      ...
      <DialogContent>
        ...
        workflow.input_questions.length > 0 ? (
          ...
          <div>
            <Typography variant="body1" color="textSecondary">
              At least one node in this workflow requires an execution argument
              ($exec). Please select one below, or provide a custom one in the
              text field next to the run button.
            </Typography>
  ...
  const EdgeSidebar = () => {
    ...
    return (
      <div style={appApiViewStyle}>
        <div style={{}}>
          <h3 style={{ marginBottom: 5 }}>Conditions</h3>
          <a
            rel="noopener noreferrer"
            target="_blank"
  ...
  const EmailSidebar = () => {
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (branch === undefined || branch === null) {
              toast("No startnode connected to node. Connect it to an action.");
              return;
            }
    ...
    const outlookButton =
      selectedTrigger.name !== "Office365" ? null : (
        ...
        >
          <img alt="" style={{ margin: 0 }} src="/images/ms_symbol_dark.svg" />
          <Typography style={{ margin: 0, marginLeft: 10 }} variant="body1">
              ...
              <span>
                ...
                {triggerFolders.length === 0 ? (
                  <Typography variant="body2">
                    No folders found. Please authenticate first. If this
                    persists,{" "}
                    <a
                      href="https://shuffler.io/contact"
                      rel="noopener noreferrer"
  ...
  const handleWorkflowSelectionUpdate = (e, isUserinput) => {
    ...
    if (
      workflow.triggers[selectedTriggerIndex].parameters[paramIndex] ===
        undefined ||
      workflow.triggers[selectedTriggerIndex].parameters[paramIndex] === null
    ) {
      workflow.triggers[selectedTriggerIndex].parameters[paramIndex] = {
        name: "subflow",
        value: "",
    ...
    setUpdate(Math.random());
    workflow.triggers[selectedTriggerIndex].parameters[paramIndex].value =
      e.target.value.id;
  ...
  const AppAuthSelector = ({ appAuthData }) => {
    ...
    const handleShowingValue = (appName) => {
      let mappingWithName = {};
      let listWithValues = workflow.triggers[
        ...
        .map((e) => e.split("="));
      if (
        listWithValues === undefined ||
        listWithValues === null ||
        listWithValues.length === 0
      ) {
        return "no-overrides";
      }
    ...
    const handleSelectChange = (appName, appId, event) => {
      ...
      const auth = transformedAuthData[appId].find(
        (auth) => auth.id === authId
      );
      ...
      if (
        workflow.triggers[selectedTriggerIndex].parameters[5] === undefined ||
        workflow.triggers[selectedTriggerIndex].parameters[5] === null
      ) {
  ...
  const SubflowSidebar = () => {
    ...
    const handleItemClick = (values) => {
      ...
      if (selectedTrigger.name === "Shuffle Workflow") {
        const toComplete =
          selectedTrigger.parameters[1].value + "$" + values[0].autocomplete;
      ...
      return (
        <div style={appApiViewStyle}>
          <span style={{ display: "flex" }}>
            ...
            <Tooltip
              title="Choose the type of subflow to run. This is NOT required, but is used to help Shuffle's workflow generators better understand the workflow."
              placement="top"
            >
              <Select
                MenuProps={{
                ...
                value={
                  selectedTrigger.tags !== undefined &&
                  selectedTrigger.tags !== null &&
                  selectedTrigger.tags.length > 0
                    ? selectedTrigger.tags[0]
                    : "Any"
                }
                onChange={(event) => {
                  if (
                  ...
                  <span>
                    ...
                    <TextField
                      ...
                      onChange={(event) => {
                        ...
                        selectedTrigger.execution_delay = parseInt(
                          event.target.value
                        );
                        setSelectedTrigger(selectedTrigger);
                      }}
                        ...
                        >
                          <Paper
                            style={{
                              ...
                              maxWidth: 275,
                              position: "fixed",
                              top: menuPosition.top - 200,
                              left: menuPosition.left - 455,
                          ...
                          >
                            ...
                            {parsedPaths.map((pathdata, index) => {
                              ...
                              const indentation_count =
                                (pathdata.name.match(/\./g) || []).length + 1;
                              const baseIndent = (
                                <div
                                  style={{
                                    marginLeft: 20,
      ...
      return (
        <div style={appApiViewStyle}>
          ...
          <TextField
            ...
            color="primary"
            placeholder={
              "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSr69kkDcJiR4Vm59ypxhGkD1JDIV0oLVDiBQ&usqp=CAU"
            }
  ...
  if (
    ...
    workflow.triggers !== null &&
    workflow.triggers !== undefined &&
    workflow.triggers.length >= selectedTriggerIndex &&
  ...
  const WebhookSidebar =
    Object.getOwnPropertyNames(selectedTrigger).length === 0 ||
    workflow.triggers[selectedTriggerIndex] === undefined ||
    selectedTrigger.trigger_type !== "WEBHOOK" ? null : (
      <div style={appApiViewStyle}>
        <h3 style={{ marginBottom: "5px" }}>
          {selectedTrigger.app_name}: {selectedTrigger.status}
        </h3>
          ...
          <div style={{ marginTop: 35 }}>
            <Autocomplete
              ...
              renderOption={(props, app, state) => {
                ...
                return (
                  <Tooltip color="secondary" title={appname} placement="left">
                    <MenuItem
                      onClick={() => {
                        ...
                        if (newValue !== undefined && newValue !== null) {
                          ...
                          if (cy !== undefined && cy !== null) {
                            const foundnode = cy.getElementById(
                              selectedTrigger.id
                            );
                ...
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton
                      ...
                      onClick={() => {
                        var copyText =
                          document.getElementById("webhook_uri_field");
                        if (copyText !== undefined && copyText !== null) {
                          ...
                          console.log(
                            "Couldn't find webhook URI field: ",
                            copyText
                          );
                        }
                      }}
            ...
            {workflow.triggers[selectedTriggerIndex].parameters.length > 4 ? (
              ...
              >
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={workflow.triggers[
                        selectedTriggerIndex
                      ].parameters[4].value.includes("v2")}
                      disabled={selectedTrigger.status === "running"}
                      onChange={(e) => {
                        if (selectedTrigger.parameters === null) {
                          selectedTrigger.parameters = [];
                  ...
                  label={
                    <div style={{ color: "white" }}>Wait For Response</div>
                  }
                />
              </FormGroup>
            ) : null}
  ...
  const newWebhook = (trigger) => {
    ...
    const await_resp = trigger.parameters.find(
      (param) => param.name === "await_response"
    );
    var version = "";
  ...
  const UserinputSidebar =
    Object.getOwnPropertyNames(selectedTrigger).length === 0 ||
    workflow.triggers[selectedTriggerIndex] === undefined ||
    selectedTrigger.trigger_type !== "USERINPUT" ? null : (
      <div style={appApiViewStyle}>
        ...
        <div style={{ flex: "6", marginTop: 10 }}>
          ...
          >
            <div style={{ flex: "10" }}>
              <b>Input options</b>
              <Typography variant="body2" color="textSecondary">
                Use subflows to connect to any app you want, or use the default
              ...
              control={
                <Checkbox
                  ...
                  checked={
                    ...
                    workflow.triggers[selectedTriggerIndex].parameters !==
                      undefined &&
                    workflow.triggers[selectedTriggerIndex].parameters.length >
                      0 &&
                    workflow.triggers[selectedTriggerIndex].parameters[2] !==
                      undefined &&
                    workflow.triggers[selectedTriggerIndex].parameters[2]
          ...
          {workflow.triggers[selectedTriggerIndex].parameters[2] !==
            undefined &&
          workflow.triggers[selectedTriggerIndex].parameters[2].value.includes(
            "subflow"
          ) ? (
            <div style={{}}>
              {workflows === undefined ||
              ...
              <Button
                ...
                disabled={
                  !(
                    subworkflow === null ||
                    subworkflow === undefined ||
                    Object.getOwnPropertyNames(subworkflow).length === 0
                  )
                }
                onClick={() => {
        ...
        workflow.input_questions.length > 0 ? (
          <div>
            {workflow.input_questions.map((question, index) => {
              var foundParamIndex = workflow.triggers[
                selectedTriggerIndex
              ].parameters.findIndex(
                (param) => param.name === "input_questions"
              );
              ...
              const selectionClick = () => {
                ...
                if (
                  workflow.triggers[selectedTriggerIndex].parameters[
                    foundParamIndex
                  ].value.includes(question.name)
                ) {
                  workflow.triggers[selectedTriggerIndex].parameters[
                    foundParamIndex
                  ].value = workflow.triggers[selectedTriggerIndex].parameters[
                    foundParamIndex
                  ].value.filter((item) => item !== question.name);
                } else {
                  workflow.triggers[selectedTriggerIndex].parameters[
                    foundParamIndex
                  ].value.push(question.name);
                }
          ...
          <div
            style={{ cursor: "pointer", color: "#f85a3e", marginTop: 10 }}
            onClick={() => {
          ...
          >
            <Typography variant="body2">
              No Input-Questions found. Click to add them!
            </Typography>
          </div>
        )}
      </div>
  ...
  if (
    ...
    selectedTrigger.environment === "onprem" &&
  ...
  const PipelineSidebar =
    Object.getOwnPropertyNames(selectedTrigger).length === 0 ||
    (workflow.triggers[selectedTriggerIndex] === undefined &&
      selectedTrigger.trigger_type !== "SCHEDULE") ? null : (
      <div style={appApiViewStyle}>
        <h3 style={{ marginBottom: "5px" }}>
          {selectedTrigger.app_name}: {selectedTrigger.status}
        </h3>
            ...
            >
              <FormControlLabel
                ...
                label={
                  ...
                  selectedTrigger.status === "running"
                    ? "listening at 192.168.1.100:5162"
          ...
          <div>
            ...
            <TextField
              ...
              defaultValue={
                selectedTrigger.parameters === undefined
                  ? ""
                  : selectedTrigger.parameters[0]?.value
              }
              color="primary"
              placeholder=""
              onBlur={(e) => {
            ...
            <TextField
              ...
              disabled={
                workflow.triggers[selectedTriggerIndex] === null ||
                workflow.triggers[selectedTriggerIndex] === undefined
                  ? false
                  : workflow.triggers[selectedTriggerIndex].status === "running"
              }
              ...
              defaultValue={
                ...
                workflow.triggers[selectedTriggerIndex].parameters !==
                  undefined &&
                workflow.triggers[selectedTriggerIndex].parameters !== null &&
                workflow.triggers[selectedTriggerIndex].parameters.length > 1
                  ? workflow.triggers[selectedTriggerIndex].parameters[1].value
                  : ""
              }
  ...
  const TopCytoscapeBar = (props) => {
    ...
    const isCorrectOrg =
      ...
      workflow.org_id === undefined ||
      workflow.org_id.length === 0 ||
      userdata.active_org.id === workflow.org_id;
      ...
      <div style={topBarStyle}>
        ...
        >
          ...
          {!distributedFromParent ? (
            isCorrectOrg ? null : (
              <Typography variant="body1">
                ...
                <span
                  ...
                  onClick={() => {
                    toast(
                      "Changing to correct organisation. Please wait a few seconds."
                    );

                    localStorage.setItem("globalUrl", "");
                    localStorage.setItem("getting_started_sidebar", "open");
                    fetch(
                      `${globalUrl}/api/v1/orgs/${workflow.org_id}/change`,
                      {
                        mode: "cors",
                        credentials: "include",
                        crossDomain: true,
                      ...
                      .then(function (responseJson) {
                        ...
                        if (responseJson.success === true) {
                          ...
                          toast(
                            "Successfully changed active organisation - refreshing!"
                          );
                        } else {
                          if (
                            responseJson.reason !== undefined &&
            ...
            <Typography variant="body1">
              Warning: This workflow is controlled by your parent org and may
              not be editable.
            </Typography>
          )}
          ...
          originalWorkflow.suborg_distribution === null ||
          originalWorkflow.suborg_distribution.length === 0 ||
          originalWorkflow.suborg_distribution.includes("none") ? null : (
            <FormControl
              fullWidth
              style={{ marginTop: 10, maxWidth: 250, pointerEvents: "auto" }}
            >
              <InputLabel id="suborg-changer" style={{ color: "white" }}>
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (
                    originalWorkflow.childorg_workflow_ids === undefined ||
                    originalWorkflow.childorg_workflow_ids === null ||
                    originalWorkflow.childorg_workflow_ids.length === 0
                  ) {
                    console.log(
                      "In childorg no exist. Suborgworkflows: ",
                      suborgWorkflows
                    );

                    if (
                      suborgWorkflows !== undefined &&
                      suborgWorkflows !== null &&
                      ...
                      if (!found) {
                        console.log(
                          "No workflow found out of suborg workflows."
                        );
                    ...
                    for (var childorgidkey in originalWorkflow.childorg_workflow_ids) {
                      const childworkflowid =
                        originalWorkflow.childorg_workflow_ids[childorgidkey];
                      for (var suborgWorkflowKey in suborgWorkflows) {
                        const suborgWorkflow =
                          suborgWorkflows[suborgWorkflowKey];
                        if (suborgWorkflow.org_id === e.target.value) {
                          workflowFound = true;
                ...
                {originalWorkflow.suborg_distribution.map((org_id, index) => {
                  ...
                  return (
                    <MenuItem key={index} value={data.id}>
                      {image} <span style={{ marginLeft: 8 }}>{data.name}</span>
                    </MenuItem>
                  );
                })}
              </Select>
          ...
          authGroups.length > 0 ? (
            ...
            >
              <span style={{ pointerEvents: "auto" }}>
                  ...
                  onMouseEnter={() => {
                    if (cy !== undefined && cy !== null) {
                      ...
                      for (var cynodekey in cynodes) {
                        ...
                        if (
                          cynodedata.data.authentication_id === "authgroups"
                        ) {
                          const foundnode = cy.getElementById(
                            cynodedata.data.id
                          );
                ...
                >
                  <LockOpenIcon style={{ marginRight: 10 }} /> Auth Group ({" "}
                  {workflow.auth_groups !== undefined &&
                  workflow.auth_groups !== null
                    ? workflow.auth_groups.length
                    : 0}
                  /{authGroups.length} )
                </Button>
              </span>
                ...
                <Tooltip
                  ...
                  title={
                    <span style={{}}>
                      ...
                      <Typography>Parent workflow: '{wf.name}'</Typography>
                    </span>
                  }
                  placement="bottom"
                >
          ...
          <span>
            ...
            >
              <SettingsIcon />
				...
				*/}
        {isMobile ? (
          <Tooltip color="secondary" title="Show apps" placement="top-start">
            <span>
              <Button
                disabled={workflow.public}
                color="primary"
  ...
  const shownErrors =
    !distributedFromParent &&
    !isMobile &&
    workflow.errors !== undefined &&
    workflow.errors !== null &&
                        ...
                        <span
                          ...
                          onClick={() => {
                            ...
                            const foundnode = cy.nodes().filter((node) => {
                              ...
                              return (
                                nodelabel.toLowerCase() === word.toLowerCase()
                              );
                            });
  ...
  const updateCurrentWorkflow = (inputworkflow) => {
    ...
    if (
      inputworkflow.parentorg_workflow !== undefined &&
      inputworkflow.parentorg_workflow !== null &&
      inputworkflow.parentorg_workflow !== ""
    ) {
      ...
      <div style={bottomBarStyle}>
        ...
        >
          ...
          <Tooltip
            ...
            title={
              workflow.public === true
                ? "Use this Workflow in your organisation"
                : "Save Workflow"
            }
            placement="top"
          >
  ...
  const unPublishWorkflow = (data) => {
    ...
    fetch(url, {
      ...
      .catch((error) => {
        toast(
          "Failed publishing: is the workflow valid? Remember to save the workflow first."
        );
        console.log(error.toString());
      });
  };
    ...
    workflow.public === true ? (
      <div
        style={{
          ...
          maxHeight: "82vh",
          height: "100%",
          minWidth: leftBarSize - 70,
          maxWidth: leftBarSize - 70,
          zIndex: 0,
          padding: 35,
          borderRight: "1px solid rgba(91,96,100,1)",
          overflowY: "auto",
        }}
      >
        <span style={{ display: "flex" }}>
          ...
          {workflow.validated === true ? (
            <Tooltip
              title="The functionality of this workflow manually verified by the Shuffle automation team"
              placement="top"
            >
        ...
        <Typography variant="body2" color="textSecondary">
          ...
          <span
            style={{ color: "#f86a3e", cursor: "pointer" }}
            onClick={() => {
              saveWorkflow(workflow);
            }}
          >
        ...
        {workflow.blogpost !== undefined &&
        workflow.blogpost !== null &&
        workflow.blogpost.length > 0 ? (
          <div
            style={{
              marginTop: 10,
              maxWidth: "100%",
        ...
        workflow.video.length > 0 ? (
          ...
          >
            ...
            {workflow.video.includes("loom.com/share") &&
            workflow.video.split("/").length > 4 ? (
              <div>
                <iframe
                  src={`https://www.loom.com/embed/${
                    workflow.video.split("/")[4]
                  }`}
                  frameBorder={"false"}
            ...
            ) : workflow.video.includes("youtube.com") &&
              workflow.video.split("/").length > 3 &&
              workflow.video.includes("v=") ? (
              <div>
                <iframe
              ...
              <Typography variant="body1">{workflow.video}</Typography>
            )}
          </div>
        ) : null}
        ...
        workflow.description.length > 0 ? (
          <div style={{ marginTop: 5 }}>
            ...
            <Typography
              variant="body1"
              color="textSecondary"
              style={{ maxWidth: "100%", maxHeight: 250, overflowX: "hidden" }}
            >
        ...
        (userdata.avatar === creatorProfile.github_avatar ||
          allowList.includes(userdata.public_username)) ? (
          <div style={{ marginTop: 50 }}>
            <Typography variant="body2" color="textSecondary">
              You can see these buttons because you may have the correct access
              rights as a creator to help modify this workflow.
            </Typography>
            <Button
              color="primary"
              variant="contained"
            ...
            <Button
              ...
              onClick={() => {
                ...
                // setUpdate(Math.random());
                toast(
                  "Unpublishing this workflow from the search engine. It will remain public until it is deleted, as it is a copy of the original workflow."
                );
            ...
            {userdata.support === true ? (
              <span>
                ...
                >
                  Manual Verification:{" "}
                  <b>
                    {workflow.validated === undefined ||
                    workflow.validated === null ||
        ...
        <div style={{ display: "flex" }}>
          ...
          <ReactJson
            ...
            collapseStringsAfterLength={
              theme.palette.jsonCollapseStringsAfterLength
            }
  ...
  const HandleJsonCopy = (base, copy, base_node_name) => {
    ...
    if (
      base_node_name === "execution_argument" ||
      base_node_name === "Execution Argument"
    ) {
      base_node_name = "exec";
    }
      ...
      <span>
        ...
        {anchorPosition !== null ? (
          <Popover
            ...
            disableRestoreFocus
          >
            <Typography style={{ padding: 5 }}>Copying</Typography>
          </Popover>
        ) : null}
      </span>
    );
  ...
  const changeExecution = (data) => {
    ...
    if (
      ...
      if (
        data.execution_argument !== undefined &&
        data.execution_argument !== null &&
        data.execution_argument.includes("too large")
      ) {
        setExecutionData({});
        checkStarted = true;
        start();
  ...
  if (executionData.results !== undefined) {
    const newSkipped = executionData.results.find(
  ...
  const envStatus = !(
    executionData.workflow !== undefined &&
    executionData.workflow !== null &&
    executionData.workflow.actions !== undefined &&
    executionData.workflow.actions !== null &&
    executionData.workflow.actions.length > 0
  )
    ? "loading"
    : "success";
  ...
  const executionModal = (
    ...
    >
      ...
      {executionModalView === 0 ? (
        <div
          style={{ padding: isMobile ? "0px 0px 0px 10px" : 25, zIndex: 12502 }}
        >
          <div style={{ display: "flex" }}>
            <Breadcrumbs
              aria-label="breadcrumb"
            ...
            >
              <a
                target="_blank"
                href={`/workflows/debug?workflow_id=${workflow.id}`}
          ...
          {workflowExecutions.length > 0 ? (
            <div>
              {workflowExecutions.map((data, index) => {
                ...
                if (
                  data.workflow.triggers !== undefined &&
                  data.workflow.triggers !== null
                ) {
                ...
                const foundnotifications =
                  data.notifications_created === undefined ||
                  data.notifications_created === null
                    ? 0
                          ...
                          onClick={() => {
                              ...
                              if (
                                ...
                                if (
                                  ...
                                  for (let resultkey in data.results) {
                                    ...
                                    if (
                                      data.results[resultkey].result.includes(
                                        "too large"
                                      )
                                    ) {
                                      setExecutionData({});
                            ...
                            if (!checkStarted) {
                              ...
                              if (cy !== undefined && cy !== null) {
                                ...
                                for (let actionKey in data.workflow.actions) {
                                  var actionitem =
                                    data.workflow.actions[actionKey];
                          ...
                          <div style={{ display: "flex", flex: 1 }}>
                            <div
                              style={{
                                marginLeft: 0,
                                width:
                                  lastExecution === data.execution_id ? 4 : 2,
                            ...
                            <Tooltip
                              ...
                              title={
                                `Execution Source: ${
                                  data.execution_source === "default"
                                    ? "manual run"
                                    : data.execution_source
                                }.` +
                                (data.authgroup !== undefined &&
                                data.authgroup !== null &&
                                data.authgroup.length > 0
                            ...
                            >
                              <ErrorOutlineIcon
                                ...
                                onClick={(e) => {
                                  ...
                                  e.stopPropagation();
                                  window.open(
                                    `/admin?admin_tab=priorities&workflow=${data.workflow.id}&execution_id=${data.execution_id}`,
                                    "_blank"
                                  );
                                }}
                              />
        ...
        <div
          style={{
            padding: isMobile ? "0px 10px 25px 10px" : "25px 15px 25px 15px",
        ...
        >
              ...
              <span style={{}}>
                <Button
                  ...
                  onClick={() => {
                    ...
                    var nextindex = -1;
                    const currentIndex = workflowExecutions.findIndex(
                      (item) => item.execution_id === executionData.execution_id
                    );
                    if (currentIndex === -1) {
                    ...
                    if (nextindex < 0) {
                      toast.warn(
                        "Use the workflow run debugger to dig deeper - nothing more to show here."
                      );
                      return;
                    }
              ...
              >
                <span style={{}}>
                  <Button
                    ...
                    disabled={
                      userdata.region_url !== "https://shuffler.io" ||
                    ...
                    onClick={() => {
                      ...
                      setTimeout(() => {
                        window.open(
                          `/api/v1/workflows/search/${executionData.execution_id}`,
                          "_blank"
                        );
                      }, 250);
                    }}
          ...
          (executionData.authgroup !== undefined &&
            executionData.authgroup !== null &&
            executionData.authgroup.length > 0) ? (
            <div style={{ display: "flex", marginLeft: 10 }}>
              <Typography variant="body1">
                <b>Source &nbsp;&nbsp;</b>
              </Typography>
              <Typography variant="body1" color="textSecondary">
                {executionData.execution_source === "authgroups" ||
                (executionData.authgroup !== undefined &&
                  executionData.authgroup !== null &&
                  executionData.authgroup.length > 0) ? (
                  <a
                  ...
                  executionData.execution_source === "web" ? (
                  <a
                    rel="noopener noreferrer"
          ...
          {executionData.started_at !== undefined ? (
            <div style={{ display: "flex", marginLeft: 10 }}>
              ...
              <Typography variant="body1" color="textSecondary">
                {new Date(executionData.started_at * 1000).toLocaleString(
                  "en-GB"
                )}
              </Typography>
            </div>
          ...
          executionData.completed_at > 0 ? (
            <div style={{ display: "flex", marginLeft: 10 }}>
              ...
              <Typography variant="body1" color="textSecondary">
                {new Date(executionData.completed_at * 1000).toLocaleString(
                  "en-GB"
                )}
              </Typography>
            </div>
          ...
          executionData.execution_argument !== null &&
          ...
          <div style={{ display: "flex", marginTop: 10, marginBottom: 30 }}>
            <div>
              ...
              !(
                ...
                <div style={{}}>
                  <CircularProgress
                    style={{ marginLeft: 145, marginBottom: 10 }}
                    onClick={() => {
                      console.log(
                        environments,
                        defaultEnvironmentIndex,
                        nonskippedResults
                      );
                    }}
                  />
                  ...
                  defaultEnvironmentIndex < environments.length &&
                  nonskippedResults.length === 0 &&
                  environments[defaultEnvironmentIndex].Name !== "Cloud" ? (
                    <Typography
                      variant="body2"
                      color="textSecondary"
                      style={{}}
                    >
                      No results yet. Is Orborus running for the "
                      {environments[defaultEnvironmentIndex].Name}" environment?{" "}
                      <a
                        href="/admin?tab=environments"
                        rel="noopener noreferrer"
                        target="_blank"
                        style={{ textDecoration: "none", color: "#f86a3e" }}
                      >
                      ...
                      . If the Workflow doesn't start within 30 seconds with
                      Orborus running, contact support:{" "}
                      <a
            ...
            <div style={{}}>
              ...
              environments[defaultEnvironmentIndex].Name !== "Cloud" ? (
                <Typography variant="body2" color="textSecondary" style={{}}>
                  ...
                  . If the Workflow doesn't start within 30 seconds with Orborus
                  running, contact support:{" "}
                  <a
                    href="mailto:support@shuffler.io"
                    rel="noopener noreferrer"
                    target="_blank"
            ...
            executionData.results.map((data, index) => {
              ...
              if (
                ...
                if (
                  ...
                  for (let actionkey in workflow.actions) {
                    if (
                      workflow.actions[actionkey].app_name ===
                        data.action.app_name ||
                      workflow.actions[actionkey].id === data.action.id ||
                      workflow.actions[actionkey].label === data.action.label ||
                      workflow.actions[actionkey].name === data.action.name
                    ) {
                      if (
                        workflow.actions[actionkey].large_image !== undefined &&
                        workflow.actions[actionkey].large_image !== null &&
                        workflow.actions[actionkey].large_image.length > 0
                      ) {
              ...
              if (
                data.similar_actions !== undefined &&
                data.similar_actions !== null
              ) {
                var minimumMatch = 85;
                  ...
                  for (let [k, kval] in Object.entries(data.similar_actions)) {
                    if (data.similar_actions.hasOwnProperty(k)) {
                      if (data.similar_actions[k].similarity > minimumMatch) {
                        matching_executions.push(
                          data.similar_actions[k].execution_id
                        );
                      }
                    }
                  }
              ...
              const highlightNode =
                chosenNodeId !== null &&
                chosenNodeId !== undefined &&
                chosenNodeId !== "" &&
                chosenNodeId === data.action.id;
                  ...
                  <div style={{ display: "flex" }}>
                    <div style={{ display: "flex", marginBottom: 15 }}>
                      <IconButton
                        ...
                        onClick={() => {
                          if (cy !== undefined) {
                            const oldstartnode = cy.getElementById(
                              data.action.id
                            );
                      ...
                      <div>
                        ...
                        >
                          <b>
                            {data.action.label === undefined ||
                            data.action.label === null ||
                            data.action.label === ""
                              ? data.action.label
                              : data.action.label.replaceAll("_", " ")}
                          </b>
                        </div>
  ...
  const AppResultVariable = ({ data, action }) => {
    ...
    if (
      data.name === "shuffle_action_logs" &&
      data.value !== undefined &&
      data.value !== null &&
      data.value.length > 0 &&
      data.value.includes("add env SHUFFLE_LOGS_DISABLED")
    ) {
      return (
        <div style={{ maxWidth: 600, marginTop: 15, overflowX: "hidden" }}>
          ...
          <Typography variant="body2" style={{ whiteSpace: "pre-line" }}>
            Logs for an action are not available without{" "}
            <a
              style={{ color: "#f85a3e" }}
              href="/admin?tab=environments"
              target="_blank"
            ...
            environment variable set to false: SHUFFLE_LOGS_DISABLED=false. Logs
            are enabled by default, except in scale mode.
          </Typography>
        </div>
      );
    }
    ...
    if (data.name.endsWith("-Url")) {
      ...
      if (
        ...
        (data.value.startsWith("/") && data.value.includes("?"))
      ) {
        showlink = true;
      }
    }
  ...
  const getErrorSuggestion = (result) => {
    ...
    if (result.success === true && result.status === 200) {
      if (result.body !== undefined && result.body !== null) {
        ...
        if (
          (stringbody.startsWith("{") && stringbody.endsWith("}")) ||
          (stringbody.startsWith("[") && stringbody.endsWith("]"))
        ) {
          return "";
        }
    ...
    if (
      ...
      (result.url.includes("192.168") ||
        result.url.includes("172.16") ||
        result.url.includes("10.0"))
    ) {
    ...
    if (stringjson.includes("invalidurl")) {
      ...
      return (
        "The URL is invalid. Change the URL to a valid one, and try again. " +
        additionalinfo
      );
    }
  ...
  const codePopoutModal = !codeModalOpen ? null : (
    <Dialog
      ...
      style={{
        pointerEvents: "none",
        zIndex: activeDialog === "result" ? 1200 : 1100,
      }}
      hideBackdrop={true}
      open={codeModalOpen}
      PaperProps={{
    ...
    >
      ...
      <span
        id="top_bar"
        style={{ position: "sticky", top: -30, zIndex: 12000 }}
      >
        <Tooltip
          ...
          <IconButton
            ...
            onClick={(e) => {
              ...
              if (executionIndex !== -1) {
                ...
                if (
                  workflowExecutions[executionIndex] !== undefined &&
                  workflowExecutions[executionIndex] !== null &&
                  workflowExecutions[
                    executionIndex
                  ].execution_argument.includes("too large")
                ) {
                  //checkStarted = true
                  setExecutionData({});
                  start();
        ...
        <Tooltip title="Move window" placement="top" style={{ zIndex: 10011 }}>
          <IconButton
      ...
      <div style={{ marginBottom: 40 }}>
            ...
            <img
              ...
              src={
                selectedResult === undefined
                  ? theme.palette.defaultImage
                  : selectedResult.action.app_name === "shuffle-subflow"
                  ? triggers[2].large_image
                  : selectedResult.action.app_name === "User Input"
                  ? triggers[3].large_image
                  : selectedResult.action !== undefined &&
                    selectedResult.action.large_image !== undefined &&
                    selectedResult.action.large_image !== null &&
                    selectedResult.action.large_image !== ""
                  ? selectedResult.action.large_image
                  : curapp !== undefined
                  ? curapp.large_image
                  : theme.palette.defaultImage
        ...
        {currentSuggestion.length > 0 ? (
          <div style={{ marginBottom: 5 }}>
            <b style={{ color: "rgba(214,110,117)" }}>Debug:</b>{" "}
            {currentSuggestion}
          </div>
        ) : (
          <div style={{ marginBottom: 5 }}>
        ...
        <div>
          ...
          selectedResult.action.parameters !== undefined ? (
            <div>
              ...
              >
                Variables{" "}
                <span style={{ fontSize: 10 }}>(click to expand)</span>
              </Typography>
  ...
  const ExecutionVariableModal = (props) => {
    ...
    const [newVariableName, setNewVariableName] = React.useState(
      variableInfo.name !== undefined ? variableInfo.name : ""
    );
    const [newVariableDescription, setNewVariableDescription] = React.useState(
      variableInfo.description !== undefined ? variableInfo.description : ""
    );
    const [newVariableValue, setNewVariableValue] = React.useState(
      variableInfo.value !== undefined ? variableInfo.value : ""
    );
              ...
              onClick={() => {
                ...
                if (found !== -1) {
                  ...
                  if (newVariableValue.length > 0) {
                    workflow.execution_variables[found].value =
                      newVariableValue;
                  }
                } else {
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (
                  ...
                  if (
                    variableInfo.index !== undefined &&
                    variableInfo.index !== null &&
                    variableInfo.index >= 0
                  ) {
                    if (newVariableName.length > 0) {
                      workflow.workflow_variables[variableInfo.index].name =
                        newVariableName;
                    }
                    ...
                    if (newVariableValue.length > 0) {
                      workflow.workflow_variables[variableInfo.index].value =
                        newVariableValue;
                    }
                ...
                if (!handled) {
                  ...
                  if (found !== -1) {
                    ...
                    if (newVariableValue.length > 0) {
                      workflow.workflow_variables[found].value =
                        newVariableValue;
                    }
                  } else {
      ...
      for (let paramkey in selectedApp.authentication.parameters) {
        if (
          ...
          if (
            ...
            if (
              selectedApp.authentication.parameters[paramkey].schema.type ===
              "bool"
            ) {
              authenticationOption.fields[
                selectedApp.authentication.parameters[paramkey].name
      ...
      >
        ...
        <div
          style={{
            height: 75,
            width: "100%",
            background: `linear-gradient(to right, #f86a3e, #fc3922)`,
            position: "relative",
          }}
        />
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "row" }}>
        ...
        >
          ...
          authenticationType.type === "oauth2-app" ? (
        ...
        >
          ...
          selectedApp.documentation.length === 0 ? (
            <span style={{ textAlign: "center" }}>
              ...
              >
                <Typography variant="h6" style={{ marginBottom: 25 }}>
                  There is no Shuffle-specific documentation for this app yet.
                  Documentation is written custom for each app, and is a
                  community effort. Hope to see your contribution
                </Typography>
                <Button
                  variant="contained"
                  color="primary"
                  onClick={() => {
                    toast.success(
                      "Opening remote Github documentation link. Thanks for contributing!"
                    );

                    setTimeout(() => {
                      window.open(
                        `https://github.com/Shuffle/openapi-apps/new/master/docs?filename=${selectedApp.name.toLowerCase()}.md`,
                        "_blank"
                      );
                    }, 2500);
                  }}
              ...
              <Typography variant="body1" style={{ marginTop: 25 }}>
                Want to help the making of, or improve this app? <br />
                <a
                  rel="noopener noreferrer"
                  target="_blank"
            ...
            <div>
              ...
              selectedMeta !== null &&
              Object.getOwnPropertyNames(selectedMeta).length > 0 &&
              selectedMeta.name !== undefined &&
              selectedMeta.name !== null ? (
                <div
                  style={{
                    backgroundColor: theme.palette.inputColor,
                    padding: 15,
    ...
    return (
      <div
        style={{
          ...
          padding: 15,
          position: "fixed",
          top: suggestionBox.position.top,
          left: suggestionBox.position.left,
          borderRadius: theme.palette.borderRadius,
          backgroundColor: theme.palette.surfaceColor,
          border: "1px solid rgba(255,255,255,0.3)",
        }}
      >
        {/*
				...
				*/}
        <Tooltip title="Close" placement="top" style={{ zIndex: 10011 }}>
          <IconButton
  ...
  const RevisionBox = (props) => {
    ...
    const validTimestamp =
      newrevision.edited.toString().length === 10
        ? newrevision.edited * 1000
        : newrevision.edited;
    const translatedDate = new Date(validTimestamp).toLocaleString("en-US", {
      month: "long",
      day: "numeric",
      hour: "numeric",
      minute: "numeric",
    ...
    var workflowStatus =
      newrevision.status !== undefined &&
      newrevision.status !== null &&
      newrevision.status !== ""
        ? newrevision.status
        : "test";
    if (
      newrevision.name !== undefined &&
      newrevision.name !== null &&
      newrevision.name !== ""
    ) {
      ...
      if (
        newrevision.name.toLowerCase().includes("dev") ||
        newrevision.name.toLowerCase().includes("staging") ||
        newrevision.name.toLowerCase().includes("rollback")
      ) {
        workflowStatus = "dev";
      }

      if (
        newrevision.name.toLowerCase().includes("prod") ||
        newrevision.name.toLowerCase().includes("main")
      ) {
        workflowStatus = "prod";
      }
    }
    ...
    return (
      <Paper
        style={{
          ...
          cursor: "pointer",
          backgroundColor:
            newrevision.edited === selectedVersion.edited
  ...
  const drawerData =
    originalWorkflow !== undefined && originalWorkflow !== null ? (
        ...
        <div style={{ height: "92%" }}>
          ...
          >
            <div style={{ marginBottom: "20px" }}>
              ...
              <RevisionBox revision={selectedVersion} />
            </div>
          ...
          {allRevisions.length > 0 ? (
            ...
            >
              {allRevisions.map((revision, index) => {
                ...
                return (
                  <RevisionBox revision={revision} key={index} index={index} />
                );
              })}
            </div>
          ) : (
            <div style={{ padding: 5 }}>
              <Typography variant="body2">
                No other revisions found. Save your workflow with changes to
                create a revision.
              </Typography>
            </div>
          )}
  ...
  const workflowRevisions = !showWorkflowRevisions ? null : (
    <div style={{ position: "absolute" }}>
      ...
      >
        <div
          style={{ flex: 1, display: "flex", paddingLeft: 30, paddingTop: 20 }}
        >
  ...
  const changeActionParameterCodeMirror = (event, count, data, actionlist) => {
    ...
    if (
      selectedAction.app_name === "Shuffle Tools" &&
      selectedAction.name === "filter_list" &&
      count === 0
    ) {
    ...
    isLoaded && workflowDone ? (
      <div>
        ...
        <VariablesModal
          ...
          setVariableInfo={setVariableInfo}
        />
