...
const OrgHeaderexpanded = (props) => {
  ...
  const [SSORequired, setSSORequired] = React.useState(
    selectedOrganization.sso_config === undefined
      ? false
  ...
  const [uploadRepo, setUploadRepo] = React.useState(
    selectedOrganization.defaults === undefined
      ? ""
      : selectedOrganization.defaults.workflow_upload_repo === undefined ||
        selectedOrganization.defaults.workflow_upload_repo.length === 0
      ? ""
      : selectedOrganization.defaults.workflow_upload_repo
  );
  const [uploadBranch, setUploadBranch] = React.useState(
    selectedOrganization.defaults === undefined
      ? defaultBranch
      : selectedOrganization.defaults.workflow_upload_branch === undefined ||
        selectedOrganization.defaults.workflow_upload_branch.length === 0
      ? defaultBranch
      : selectedOrganization.defaults.workflow_upload_branch
  );
  const [uploadUsername, setUploadUsername] = React.useState(
    selectedOrganization.defaults === undefined
      ? ""
      : selectedOrganization.defaults.workflow_upload_username === undefined ||
        selectedOrganization.defaults.workflow_upload_username.length === 0
      ? ""
      : selectedOrganization.defaults.workflow_upload_username
  );
  const [uploadToken, setUploadToken] = React.useState(
    selectedOrganization.defaults === undefined
      ? ""
      : selectedOrganization.defaults.workflow_upload_token === undefined ||
        selectedOrganization.defaults.workflow_upload_token.length === 0
      ? ""
      : selectedOrganization.defaults.workflow_upload_token
  );
      ...
      .then((responseJson) => {
        if (responseJson !== undefined) {
          ...
          if (
            ...
            const workflow = responseJson.find(
              (workflow) =>
                workflow.id ===
                selectedOrganization.defaults.notification_workflow
            );
      ...
      <Grid container spacing={3} style={{ textAlign: "left" }}>
                      ...
                      <Tooltip
                        ...
                        title={
                          <span style={{}}>
                            ...
                            <Typography>Choose {data.name}</Typography>
                          </span>
                        }
                        placement="bottom"
                      >
        ...
        <Grid item xs={12} style={{ marginTop: 50 }}>
          ...
          <Typography
            ...
            color="textSecondary"
          >
            Decide where workflows are backed up in a Git repository. Will
            create logs and notifications if upload fails. The repository and
            branch must already have been initialized. Files will show up in the
            root folder in the format 'status_workflowid.json'
          </Typography>
